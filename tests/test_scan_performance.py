...
class ScanPerformanceServiceTests(unittest.TestCase):
    ...
    def test_scan_performance_writes_report(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("## Evidence Yardsticks", report_text)
            self.assertIn("annualized_sharpe_alpha", report_text)
            self.assertIn("newey_west_t", report_text)
            self.assertIn("years_for_t_1_96", report_text)
            self.assertIn("## Concentration Hygiene", report_text)
