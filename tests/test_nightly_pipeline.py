...
class NightlyPipelineScriptTests(unittest.TestCase):
    def test_shortlist_promotion_gate_failure_continues_to_downstream_steps(self) -> None:
        ...
        self.assertIn("latest_champion_date", script)
        self.assertIn("horizon_days = ?", script)
        self.assertIn("champion_model IS NOT NULL", script)
        self.assertNotIn("WHERE is_active = 1", script)
        ...
        self.assertIn("recorded_promotion_failures", script)
        self.assertIn("days_since_last_champion", script)
        self.assertIn("read_shortlist_model_config", script)
        self.assertIn("--horizon \"${shortlist_horizon_days}\"", script)
        self.assertIn("--top \"${shortlist_top_n}\"", script)
        self.assertIn("days since the last promoted champion", script)
        ...
        self.assertNotIn("--top 10", script)
